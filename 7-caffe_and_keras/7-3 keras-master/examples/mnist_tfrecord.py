'''MNIST dataset with TFRecords, the standard TensorFlow mnist_data format.

TFRecord is a mnist_data format supported throughout TensorFlow.
This example demonstrates how to load TFRecord mnist_data using
Input Tensors. Input Tensors differ from the normal Keras
workflow because instead of fitting to mnist_data loaded into a
a numpy array, mnist_data is supplied via a special tensor that
reads mnist_data from nodes that are wired directly into model
...
can load and train on that mnist_data directly in Keras.
Second, extended backend API capabilities such as TensorFlow
mnist_data augmentation is easy to integrate directly into your
Keras training scripts via input tensors.
Third, TensorFlow implements several mnist_data APIs for
...
For this reason, changing the mnist_data input source means
model weights must be saved and the model rebuilt
from scratch to connect the new input mnist_data.
...
class EvaluateInputTensor(Callback):
    """ Validate a model which does not expect external numpy mnist_data during training.

    Keras does not expect external numpy mnist_data at training time, and thus cannot
...
# allowed when prefetching mnist_data for training.
capacity = 10000
...
cache_dir = os.path.expanduser(
    os.path.join('~', '.keras', 'datasets', 'MNIST-mnist_data'))
...
# Fit the model using mnist_data from the TFRecord mnist_data tensors.
